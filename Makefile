...
N64PSP_PROFILE_PSP ?= 0
N64PSP_USE_VFPU ?= 1
N64PSP_VFPU_TRANSFORM_EXPERIMENT ?= 0
N64PSP_PSP_TEXT_LOG ?= $(if $(filter build-psp-sincos-vfpu,$(BUILD_PSP)),1,0)
N64PSP_PSP_TEXT_LOG_PATH ?= ms0:/n64psp_sincos_vfpu.txt
...
ifeq ($(N64PSP_PROFILE_PSP),1)
PSP_CFLAGS += -pg -g -fno-omit-frame-pointer -fno-optimize-sibling-calls
endif
...
ifeq ($(N64PSP_PSP_TEXT_LOG),1)
PSP_CFLAGS += -DN64PSP_PSP_TEXT_LOG=1
PSP_CFLAGS += -DN64PSP_PSP_TEXT_LOG_PATH=\"$(N64PSP_PSP_TEXT_LOG_PATH)\"
endif
