...
| ------ | --------------- | -------------------- | ------------------------- | ------------- | ------------------- | ------------------ | ----------------------- |
| `osCreateMesgQueue` | `src/psp/ultra_reimpl.c:124` | `src/audio/audio_load.c:1041`, `src/audio/audio_thread.c:254`, `src/sys/sys_timer.c`, many game queues | Initializes caller buffer; no native wait objects; `validCount` is current count, `msgCount` capacity. | Implemented | Mostly yes after queue layout fix. | Existing SF64 headers define `OSThread* mtqueue/fullqueue`; n64psp uses opaque pointer fields for ABI shape. | Blocking wake-up on PSP threads. |
| `osSendMesg` | `src/psp/ultra_reimpl.c:133` | Audio load/thread queues, VI/SP/DP event posting, save requests | Uses interrupt lock plus `sceKernelDelayThread(1000)` polling for blocking sends. | Implemented | Yes for queue semantics; n64psp is stronger because it blocks on semaphores. | Return code values are negative n64psp results instead of SF64's generic `-1`; adapter may normalize during migration. | Producer wake-up and no busy wait. |
| `osJamMesg` | `src/psp/ultra_reimpl.c:159` | `src/libultra/io/epidma.c:55` and PI command paths | Front insertion but shifts messages after moving `first`; uses polling for blocking. | Implemented | Yes for front insertion semantics. | Same return-code adapter concern. | Full queue wake-up under PSP load. |
...
| `osRecvMesg` | `src/psp/ultra_reimpl.c:198` | Audio DMA queues, SI/controller waits, VI/task queues, macros in `include/sf64thread.h:191` | Uses interrupt lock plus `sceKernelDelayThread(1000)` polling for blocking receives. | Implemented | Yes for queue semantics; n64psp avoids busy-waiting. | Return-code adapter may be needed for code expecting exactly `-1`. | Consumer wake-up and queue reinitialisation. |
...
The useful cross-port conclusion is narrow: queues and time/count wrappers are common early migration surfaces; SF64's busy-waiting PSP queue implementation is a correctness/performance shortcut that n64psp should replace with real blocking primitives.

## Directly Replaceable Today

`osCreateMesgQueue`, `osSendMesg`, `osJamMesg`, and `osRecvMesg` are the only SF64 `os*` functions close to direct replacement today. A thin adapter may still be useful to normalize SF64's generic `-1` error expectation and to include exact SF64/N64 type aliases.
